...
export default function WithdrawalsPage() {
  ...
  const { data: stats, isLoading: isLoadingStats } = useQuery<ConsultantStats>({
  ...
  const { data: withdrawals, isLoading: isLoadingWithdrawals } =
    useQuery<WithdrawalResponse>({
      queryKey: ['consultant-withdrawals', dateRange],
      queryFn: async () => {
        ...
        const response = await api.get(
          `/consultant/withdrawal/?${params.toString()}`
        );
        return response.data;
      }
    });
  ...
  const copyToClipboard = async (text: string) => {
    try {
      ...
      toast.error('Failed to copy Withdrawal ID');
    }
  };
  ...
  const totalTrend = (stats?.total_withdrawals ?? 0) > 0 ? '+12.5%' : '0%';
        ...
        <Card className='@container/card'>
          ...
          <CardFooter className='flex-col items-start gap-1.5 text-sm'>
            <div className='line-clamp-1 flex gap-2 font-medium'>
              Total withdrawals made <Banknote className='size-4' />
            </div>
          ...
          {isLoadingWithdrawals ? (
            ...
            <div className='overflow-x-auto'>
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className='w-[200px]'>Withdrawal ID</TableHead>
                ...
                <TableBody>
                  {withdrawals?.results.map((withdrawal: Withdrawal) => (
