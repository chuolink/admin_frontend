...
import type { DataCourseVideo } from '@/features/data-admin/types';
...
interface CourseVideoTableProps {
  dialogOpen: boolean;
  onDialogOpenChange: (open: boolean) => void;
}

export function CourseVideoTable({
  dialogOpen,
  onDialogOpenChange
  ...
  const [editingVideo, setEditingVideo] = useState<DataCourseVideo | null>(
    null
  );
  const [deleteOpen, setDeleteOpen] = useState(false);
  const [videoToDelete, setVideoToDelete] = useState<DataCourseVideo | null>(
    null
  );
  ...
  const form = useForm<VideoFormValues>({
    ...
    defaultValues: { course: '', name: '', video_url: '' }
  });

  useEffect(() => {
    if (dialogOpen && !editingVideo) {
      form.reset({ course: courseFilter ?? '', name: '', video_url: '' });
    }
  }, [dialogOpen, editingVideo, form, courseFilter]);

  const handleEdit = (video: DataCourseVideo) => {
  ...
  const columns = useMemo<ColumnDef<DataCourseVideo>[]>(
    () => [
      {
