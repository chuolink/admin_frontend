...
export default function EditCountryPage({
  ...
  const updateMutation = useMutation({
    mutationFn: async (values: CountryFormValues) => {
      ...
      const response = await axios.put(`/api/countries/${countryId}`, values);
      return response.data;
    },
    onSuccess: () => {
  ...
  const onSubmit = async (values: CountryFormValues) => {
    try {
      console.log('Form values before submission:', values);
      console.log('Reasons before submission:', values.reasons);
            ...
            >
              ...
              <TabsContent value='why-chuolink' className='mt-6'>
                <Card>
                  ...
                  <CardContent className='space-y-6'>
                    <div className='space-y-4'>
                      {(form.watch('reasons') || []).map((_, index) => (
                        <div key={index} className='flex gap-2'>
                          <Textarea
                            value={form.watch(`reasons.${index}`) || ''}
                            onChange={(e) => {
                              const currentReasons =
                                form.getValues('reasons') || [];
                          ...
                          <Button
                            ...
                            onClick={() => {
                              ...
                              form.setValue(
                                'reasons',
                                currentReasons.filter((_, i) => i !== index)
                              );
                            }}
                          >
                            <Trash2 className='h-4 w-4' />
                      ...
                      <Button
                        ...
                        onClick={() => {
                          const currentReasons =
                            form.getValues('reasons') || [];
                          form.setValue('reasons', [...currentReasons, '']);
                        }}
                      >
                        <Plus className='mr-2 h-4 w-4' />
                        Add Reason
