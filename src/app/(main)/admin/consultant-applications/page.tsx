...
export default function ConsultantApplicationsPage() {
  ...
  //   queryFn: async () => {
  //     if (!api || !consultant?.id) throw new Error('API not initialized or consultant not found');
  //     const response = await api.get(`/consultant/overview/${consultant.id}/stats/`);
  ...
  // const applicationTrend = (stats?.total_applications ?? 0) > 0 ? '+12.5%' : '0%';
        ...
        {/* <div className='*:data-[slot=card]:from-primary/5 *:data-[slot=card]:to-card dark:*:data-[slot=card]:bg-card grid grid-cols-1 gap-4 *:data-[slot=card]:bg-gradient-to-t *:data-[slot=card]:shadow-xs md:grid-cols-2 lg:grid-cols-4'>
          ...
          <Card className='@container/card'>
            ...
            <CardFooter className='flex-col items-start gap-1.5 text-sm'>
              ...
              <div className='flex items-center gap-1.5 text-green-600 dark:text-green-400 mt-2'>
                <Banknote className='size-4' />
                <span>Total Earnings: {formatCurrency(stats?.total_earnings || 0)}</span>
              </div>
            </CardFooter>
          </Card>
        </div> */}
